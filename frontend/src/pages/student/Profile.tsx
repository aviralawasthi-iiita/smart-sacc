...
const StudentProfile: React.FC = () => {
  ...
  const updateUserApi = async (payload: typeof form) => {
    ...
    const backendPayload = {
      ...payload,
      games: payload.games
        .filter(userGame => userGame.game?._id)
        .map(userGame => ({
      ...
      <main className="container mx-auto px-4 md:px-6 pt-24 pb-12">
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            ...
            {form.games.length > 0 ? (
              <div className="space-y-3">
                {form.games.map((userGame, idx) => {
                  if (!userGame.game) return null;
                  return (
                    <div key={userGame.game._id || idx} className="p-3 border rounded-md flex justify-between items-center">
